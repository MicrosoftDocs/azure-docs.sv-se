...
| Microsoft. Communication. ChatMessageReceived *                | Publicerad när ett meddelande tas emot för en användare i en chatt-tråd som användaren är medlem i.        |
...
* Se till att du anger "avsändar namn" i ditt "Skicka meddelande"-API-anrop för de här händelserna för att komma igång.

## <a name="event-subjects"></a>Händelse ämnen

`subject`Fältet för alla kommunikations tjänst händelser identifierar användaren, telefonnumret eller entiteten som händelsen är riktad till. Vanliga prefix används för att tillåta enkel [Event Grid filtrering](./event-filtering.md).
...
När en händelse utlöses skickar Event Grid-tjänsten data om händelsen för att prenumerera på slut punkter. Det här avsnittet innehåller ett exempel på hur data ska se ut för varje händelse.
