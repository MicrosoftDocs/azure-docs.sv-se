...
Du kan använda Azure Media Services till att leverera strömmar som krypterats med Microsoft PlayReady, Google Widevine eller Apple FairPlay-licenser. För djupgående förklaringar, se [innehålls skydd med dynamisk kryptering](drm-content-protection-concept.md).
...
![AMS med DRM-skyddad video i Azure Media Player](./media/drm-protect-with-drm-tutorial/ams_player.png)
...
* Gå igenom artikeln med [översikten om innehållsskydd](drm-content-protection-concept.md).
* Granska [innehålls skydds systemet design multi-DRM med åtkomst kontroll](architecture-design-multi-drm-system.md).
...
Som nämns ovan är objektet **Transformering** receptet och ett [Jobb](transform-jobs-concept.md) är det faktiska begärandet till Media Services om att tillämpa **transformeringen** på en indatavideo eller ett ljudinnehåll. **Jobbet** anger information som platsen för indata-videon och platsen för utdata.
...
* Konfiguration
    ...
    Licenserna för [PlayReady](drm-playready-license-template-concept.md) och [Widevine](drm-widevine-license-template-concept.md) är konfigurerade så att de kan levereras av Media Services-licensleveranstjänsten. Även om den här exempel appen inte konfigurerar [Fairplay](drm-fairplay-license-overview.md) -licensen, innehåller den en metod som du kan använda för att konfigurera Fairplay. Du kan lägga till FairPlay-konfiguration som ett annat alternativ.
...
1. Skapa en [strömmande positionerare](stream-streaming-locators-concept.md).
...
> [!IMPORTANT]
> Om du använder en anpassad [strömningsprincip](stream-streaming-policy-concept.md) bör du skapa en begränsad uppsättning av sådana principer för ditt Media Service-konto, och återanvända dem för dina StreamingLocators när samma krypterings- och protokollalternativ krävs. Media Service-kontot har en kvot för antalet StreamingPolicy-poster. Du bör inte skapa en ny StreamingPolicy för varje StreamingLocator.
...
![Skydda med DRM](./media/drm-protect-with-drm-tutorial/playready_encrypted_url.png)
...
> [Skydda med AES-128](drm-playready-license-template-concept.md)
