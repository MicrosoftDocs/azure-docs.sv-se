...
För kunder som vill leverera innehåll till stora Internet-åhörare rekommenderar vi att du aktiverar CDN på [slut punkten för direkt uppspelning](stream-streaming-endpoint-concept.md).
...
> [!NOTE]
> Du kan använda [Azure Portal](https://portal.azure.com/) för att hantera v3 [Live-händelser](live-event-outputs-concept.md), Visa v3- [till gångar](assets-concept.md), hämta information om åtkomst till API: er. För alla andra hanterings uppgifter (t. ex. transformationer och jobb) använder du [REST API](/rest/api/media/), [CLI](/cli/azure/ams)eller någon av de [SDK](media-services-apis-overview.md#sdks): er som stöds.
...
Direkt avskrift är en funktion som du kan använda med Live-händelser som antingen är direkt eller direktsänd kodning. Mer information finns i [direkt avskriftering](live-event-live-transcription-how-to.md). När den här funktionen är aktive rad använder tjänsten funktionen [tal-till-text](../../cognitive-services/speech-service/speech-to-text.md) i Cognitive Services för att skriva över talade ord i inkommande ljud till text. Den här texten görs sedan tillgänglig för leverans tillsammans med video och ljud i MPEG-streck och HLS-protokoll.
...
- [Slutpunkter för direktuppspelning](stream-streaming-endpoint-concept.md)
- [Livehändelser och liveutdata](live-event-outputs-concept.md)
...
2. Skapa en [Live-händelse](live-event-outputs-concept.md). <br/>När du skapar händelsen kan du ange att den ska autostartas. Du kan också starta händelsen när du är redo att börja strömma.<br/> När Autostart har angetts till True startas direkt sändningen direkt efter att den har skapats. Faktureringen börjar så snart direkt händelsen börjar köras. Du måste uttryckligen anropa Stop Event-resursen för att stoppa ytterligare fakturering. Mer information finns i [händelse tillstånd och fakturering](live-event-states-billing-concept.md).
3. Hämta hämtnings-URL: er och konfigurera din lokala kodare för att använda URL: en för att skicka bidrags flödet.<br/>Se [rekommenderade Live-kodare](encode-recommended-on-premises-live-encoders.md).
...
7. Skapa en **strömmande lokaliserare** med de [inbyggda typer av strömmande principer](stream-streaming-policy-concept.md).
...
- [Rekommenderade livekodare](encode-recommended-on-premises-live-encoders.md)
- [Använda en molnbaserad DVR-spelare](live-event-cloud-dvr-time-how-to.md)
- [Funktions jämförelse för live event types](live-event-types-comparison-reference.md)
- [Tillstånd och fakturering](live-event-states-billing-concept.md)
- [Svarstid](live-event-latency-reference.md)
