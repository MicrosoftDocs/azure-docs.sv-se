---
title: 'Snabbstart: Skapa en Python-app i Linux'
description: Kom igång med Azure App Service genom att distribuera en Python-app till en Linux-container i App Service.
ms.topic: quickstart
ms.date: 09/22/2020
...
I den här snabbstarten distribuerar du en [Python-webbapp till App Service på Linux](overview.md#app-service-on-linux), Azures mycket skalbara och självkorrigeringsbara webbvärdtjänst. Du använder det lokala Azure-kommandoradsgränssnittet (CLI) på en Mac-, Linux- eller [Windows-dator](/cli/azure/install-azure-cli) för att distribuera ett exempel med antingen Flask- eller Django-ramverken. Den webbapp som du konfigurerar använder App Service kostnadsfri nivå, så du får inga kostnader i den här artikeln.

> [!TIP]
> Om du föredrar att distribuera appar via en IDE kan du gå till **[Distribuera Python-appar till App Service från Visual Studio Code](/azure/developer/python/tutorial-deploy-app-service-on-linux-01)**.
...
1. Installera <a href="/cli/azure/install-azure-cli" target="_blank">Azure CLI</a> 2.0.80 eller senare, med vilken du kör kommandon i val annat gränssnitt för att etablera och konfigurera Azure-resurser.
...
Exemplet innehåller ramverksspecifik kod som Azure App Service identifierar när appen startas. Mer information finns i [Startprocess för containrar.](configure-language-python.md#container-startup-process)
...
- Du kan även inkludera argumentet där är `--location <location-name>` `<location_name>` en tillgänglig Azure-region. Du kan hämta en lista över tillåtna regioner för ditt Azure-konto genom att köra [`az account list-locations`](/cli/azure/appservice#az-appservice-list-locations) kommandot .
...
Det kan ta några minuter att slutföra kommandot. När den körs visas meddelanden om att skapa resursgruppen, App Service plan värdappen, konfigurera loggning och sedan utföra ZIP-distribution. Sedan visas meddelandet "Du kan starta appen på http:// &lt; appnamn .azurewebsites.net", som är &gt; appens URL på Azure.
...
Bläddra till det distribuerade programmet i webbläsaren på URL:en `http://<app-name>.azurewebsites.net` . Det tar en stund att starta appen från början.
...
::: zone pivot="python-framework-flask"
1. Kontrollera att du är i mappen *python-docs-hello-world.* 
...
1. Kör utvecklingsservern.
    ...
    Som standard förutsätter servern att appens inmatningsmodul finns *i app.py*, som används i exemplet. (Om du använder ett annat modulnamn anger du `FLASK_APP` miljövariabeln till det namnet.)
...
::: zone pivot="python-framework-django"
1. Kontrollera att du är i mappen *python-docs-hello-django.* 
...
Du kan också inkludera `--logs` parametern med sedan `az webapp up` kommandot för att automatiskt öppna loggströmmen vid distributionen.
...
I de föregående stegen skapade du Azure-resurser i en resursgrupp. Resursgruppen har ett namn som "appsvc_rg_Linux_CentralUS" beroende på din plats. Om du fortsätter att köra webbappen får du vissa löpande kostnader (se [App Service priser).](https://azure.microsoft.com/pricing/details/app-service/linux/)
