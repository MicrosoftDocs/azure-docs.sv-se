...
description: Översikt över de autentiseringsmetoder som registrerats och används av din organisation för att logga in och återställa lösen ord.
...
Klicka på användare som är **kompatibla med Azure Multi-Factor Authentication**, användare som kan **använda lösen ords lös autentisering** eller användare som har **möjlighet att återställa lösen ord för självbetjäning**, eller insikter om att filtrera en lista med information om användar registrering.

- **Användare som kan använda Azure Multi-Factor Authentication** visar en uppdelning av användare som kan använda MFA i Azure AD. Användare betraktas som kompatibla om de båda är registrerade för en stark autentiseringsmetod och aktive ras av en princip för att utföra MFA-metoden. Det här antalet återspeglar inte användare som registrerats för MFA utanför Azure AD. 
- **Användare med möjlighet att autentisera lösen ord** visar en uppdelning av användare som kan logga in utan lösen ord. Detta inkluderar användare som registrerats för FIDO2, Windows Hello för företag och lösen ords lös telefonin loggning med Microsoft Authenticator-appen. 
- **Användare som har en lösen ords återställning via självbetjäning** visar en uppdelning av användare som kan återställa lösen ord med självbetjäning. Användare anses vara kompatibla med SSPR om de båda är registrerade för tillräckligt många metoder för att uppfylla en organisations SSPR-princip och aktive ras för att utföra SSPR. 

  ![Skärm bild av användare som kan registrera sig](media/how-to-authentication-methods-usage-insights/users-capable.png)

**Användare som registrerats med autentiseringsmetoden** visar hur många användare som har registrerats för varje autentiseringsmetod. Klicka på en autentiseringsmetod för att se vilka användare som har registrerats för den metoden. 

![Skärm bild av registrerade användare](media/how-to-authentication-methods-usage-insights/users-registered.png)

**Senaste registrering per autentiseringsmetod** visar antalet lyckade och misslyckade autentiseringsmetoder för autentiseringsmetoden. Klicka på en autentiseringsmetod för att se de senaste registrerings händelserna för den metoden.
...
**Användnings** rapporten visar vilka autentiseringsmetoder användare använder för att logga in och återställa sina lösen ord.

![Skärm bild av sidan användning](media/how-to-authentication-methods-usage-insights/usage-page.png)

**Inloggningar efter autentiseringskrav** visar antalet lyckade användare interaktiva inloggningar som krävdes för att utföra en enkel faktor jämfört med Multi-Factor Authentication i Azure AD. Detta återspeglar inte inloggningar där MFA tillämpades av en tredjepartsleverantör av en tredje part.
...
Registreringar av tillfälliga åtkomst pass (KNACKNING) visas inte på fliken registrering i rapporten eftersom de bara är giltiga under kort tids period.
