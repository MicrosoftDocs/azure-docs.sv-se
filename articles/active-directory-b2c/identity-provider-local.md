...
1. Slutför stegen i avsnittet [Lägg till program-ID: n i avsnittet anpassad princip](tutorial-create-user-flows.md?pivots=b2c-custom-policy#add-application-ids-to-the-custom-policy) i [komma igång med anpassade principer i Azure Active Directory B2C](tutorial-create-user-flows.md?pivots=b2c-custom-policy). Du kan till exempel uppdatera `/phone-number-passwordless/` **`Phone_Email_Base.xml`** med **program-ID: n** för de två program som du registrerade när du slutfört kraven, *IdentityExperienceFramework* och *ProxyIdentityExperienceFramework*.
