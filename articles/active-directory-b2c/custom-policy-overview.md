---
title: Översikt över Azure Active Directory B2C anpassad princip | Microsoft Docs
...
En anpassad princip är helt konfigurerbar och princip driven. En anpassad princip dirigerar förtroende mellan entiteter i standard protokoll. Till exempel OpenID Connect, OAuth, SAML och några icke-standardvärden, till exempel REST API-baserade system-till-system-anspråk. Ramverket skapar användarvänliga upplevelser med vita etiketter.
...
Azure AD B2C anpassad princip [Start paket](tutorial-create-user-flows.md?pivots=b2c-custom-policy#get-the-starter-pack) innehåller flera fördefinierade principer för att komma igång snabbt. Vart och ett av dessa start paket innehåller det minsta antalet tekniska profiler och användar resor som krävs för att uppnå de scenarier som beskrivs:
...
![Diagram som visar körnings flödet för principen](./media/custom-policy-overview/custom-policy-execution.png)
...
![anpassad användar resa](media/custom-policy-overview/user-journey-flow.png)
...
![Azure AD B2C användar resa](media/custom-policy-overview/user-journey-diagram.png)
...
![Diagram över verifiering av teknisk profil](media/custom-policy-overview/validation-technical-profile.png)
...
![Diagram som visar förtroende ramverkets princip arvs modell](media/custom-policy-overview/policies.png)
...
1. Lägg till nödvändiga [princip nycklar](tutorial-create-user-flows.md?pivots=b2c-custom-policy#add-signing-and-encryption-keys) och [Registrera program för identitets upplevelse Framework](tutorial-create-user-flows.md?pivots=b2c-custom-policy#register-identity-experience-framework-applications).
1. [Hämta start paketet för Azure AD B2C policy](tutorial-create-user-flows.md?pivots=b2c-custom-policy#get-the-starter-pack) och ladda upp till din klient. 
1. När du har laddat upp start paketet [testar du din registrerings-eller inloggnings princip](tutorial-create-user-flows.md?pivots=b2c-custom-policy#test-the-custom-policy).
