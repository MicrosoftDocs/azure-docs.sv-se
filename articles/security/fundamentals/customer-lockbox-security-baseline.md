...
description: Customer Lockbox för Microsoft Azure säkerhets bas linje ger procedur vägledning och resurser för att implementera de säkerhets rekommendationer som anges i Azures säkerhets benchmark.
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 03/31/2021
...
Den här säkerhets bas linjen använder vägledning från [Azures säkerhets benchmark version 1.0](../benchmarks/overview-v1.md) till Customer lockbox. Azure Security Benchmark ger rekommendationer om hur du kan skydda dina molnlösningar i Azure. Innehållet grupperas efter de **säkerhets kontroller** som definieras av Azures säkerhets benchmark och relaterade rikt linjer som gäller för Customer lockbox. 

>[!NOTE]
>**Kontroller** som inte är tillämpliga på Customer lockbox, eller för vilka ansvaret är Microsofts, har uteslutits. Om du vill se hur Customer Lockbox helt mappar till Azures säkerhets mätning, se den [fullständiga Customer lockbox mappnings filen för säkerhets bas linjen](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
**Vägledning**: Aktivera Azure AD Multi-Factor Authentication och följ rekommendationer för Azure Security Center identitets-och åtkomst hantering.
...
**Vägledning**: Använd en privilegie rad åtkomst arbets Station (Paw) med Azure AD Multi-Factor Authentication aktive rad för att logga in på och konfigurera dina Customer Lockboxs begär Anden. 
