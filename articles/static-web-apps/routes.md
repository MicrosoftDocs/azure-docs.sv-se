...
ms.locfileid: "107313648"
---
# <a name="routes-in-azure-static-web-apps-preview"></a>Vägar i för hands versionen av Azure statisk Web Apps

> [!IMPORTANT]
> Funktioner som definieras i _routes.jspå_ filen är nu föråldrade och implementeras bättre i [konfigurations filen](./configuration.md#routes)för Azure statisk Web Apps.

Routning i Azures statiska Web Apps definierar regler för Routning och auktorisering av både statiskt innehåll och API: er<sup>1</sup>. Reglerna definieras som en uppsättning regler i _staticwebapp.config.js_ i filen.

- _staticwebapp.config.js_ filen måste finnas i roten i appens version av programartefakt.
...
- Roller definieras i _staticwebapp.config.jspå_ filen och användare är kopplade till roller via [inbjudningar](authentication-authorization.md).
...
_staticwebapp.config.js_ filen måste finnas i roten i appens version av programartefakt. Om din webbapp innehåller ett build-steg som kopierar skapade filer från en speciell mapp till din version av en artefakt, måste _staticwebapp.config.js_ filen finnas i den specifika mappen.

I följande tabell visas en lista över lämpliga platser för att lägga _staticwebapp.config.jspå_ en fil för ett antal klient dels ramverk och bibliotek.
...
Vägar definieras i _staticwebapp.config.jspå_ filen som en matris med väg regler i `routes` egenskapen. Varje regel består av ett väg mönster, tillsammans med en eller flera av de valfria regel egenskaperna. Se [exempel på väg filen](#example-route-file) för användnings exempel.
...
Användare kan stöta på ett antal olika situationer som kan resultera i ett fel. Med hjälp av `platformErrorOverrides` matrisen kan du tillhandahålla en anpassad upplevelse som svar på dessa fel. Referera till [exempel cirkulations filen](#example-route-file) för placering av matrisen i _staticwebapp.config.jsi_ filen.
...
- Sidhuvuden som definieras istaticwebapp.config.jstillämpas endast _på_ statiskt innehåll. Du kan anpassa svars rubriker för en API-slutpunkt i funktionens kod.
...
I följande exempel visas hur du skapar väg regler för statiskt innehåll och API: er i en _staticwebapp.config.jspå_ en fil. Vissa vägar använder [ _/.auth_ -systemmappen](authentication-authorization.md) som använder autentiserings-relaterade slut punkter.
...
| _/customers/contoso_               | Autentiserade användare som tillhör antingen _Administratörer_ eller _customers_contoso_ roller behandlas i _/Customers/contoso/index.html_ -fil <sup>2</sup>. Autentiserade användare som inte tillhör _Administratörer_ eller _customers_contoso_ roller betjänas av ett 401-fel. Oautentiserade användare Omdirigerad till _/login_. |
...
- _staticwebapp.config.jspå_ filen får inte vara större än 100 kB
- _staticwebapp.config.jsi_ filen stöder högst 50 distinkta roller
