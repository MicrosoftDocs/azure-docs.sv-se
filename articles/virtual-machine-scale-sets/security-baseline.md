...
Den här säkerhets bas linjen använder vägledning från [Azures säkerhets benchmark version 1.0](../security/benchmarks/overview-v1.md) till Virtual Machine Scale Sets. Azures säkerhets benchmark ger rekommendationer om hur du kan skydda dina moln lösningar på Azure. innehållet grupperas av de **säkerhets kontroller** som definieras av Azures säkerhets benchmark och relaterade rikt linjer som gäller för Virtual Machine Scale Sets.

> [!NOTE]
> **Kontroller** som inte är tillämpliga på Virtual Machine Scale Sets, eller för vilka ansvaret är Microsofts, har uteslutits. Om du vill se hur Virtual Machine Scale Sets helt mappar till Azures säkerhets mätning, se den **[fullständiga Virtual Machine Scale Sets mappnings filen för säkerhets bas linjen](https://github.com/MicrosoftDocs/SecurityBenchmarks/raw/master/Azure%20Offer%20Security%20Baselines/1.1/virtual-machine-scale-sets-security-baseline-v1.1.xlsx)**.
...
**Vägledning**: om du använder skalnings uppsättningen för virtuella datorer som värd för webb program, använder du en nätverks säkerhets grupp (NSG) i under nätet för skalnings uppsättning för virtuella datorer för att begränsa vilken nätverks trafik, vilka portar och protokoll som tillåts kommunicera. Följ en princip för minsta möjliga nätverks användning när du konfigurerar din NSG: er så att den endast tillåter obligatorisk trafik till ditt program.
...
- [Förstå Log Analytics arbets yta](/azure/azure-monitor/logs/get-started-portal)
...
**Vägledning**: skalnings uppsättningen för virtuella Azure-datorer och Azure Active Directory (Azure AD) har inte begreppet standard lösen ord. Kund ansvarig för program från tredje part och Marketplace-tjänster som kan använda standard lösen ord.
...
- [Övervaka virtuella datorer i Azure](/azure/azure-monitor/vm/monitor-vm-azur)
...
**Vägledning**: Azure Virtual Machine Scale Sets kan använda automatiska uppgraderingar av OS-avbildningar. Du kan använda Azures tillägg för önskad tillstånds konfiguration (DSC) för de underliggande virtuella datorerna i skalnings uppsättningen för den virtuella datorn. DSC används för att konfigurera de virtuella datorerna när de är online så att de kör önskad program vara.
...
**Vägledning**: beroende på typen av skript kan du använda konfigurationer för operativ system eller resurser från tredje part för att begränsa användarnas möjlighet att köra skript i Azure Compute-resurser.
...
- [Ladda upp en anpassad virtuell hård disk till Azure](../virtual-machines/windows/upload-generalized-managed.md)
