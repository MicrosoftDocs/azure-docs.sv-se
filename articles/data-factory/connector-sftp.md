...
| ***Leta upp filerna som ska kopieras** _ |  |  |
| ALTERNATIV 1: statisk sökväg<br> | Kopiera från den mapp-/fil Sök väg som anges i data uppsättningen. Om du vill kopiera alla filer från en mapp anger du även `wildcardFileName` som `_` . |  |
...
| rekursiva | Anger om data ska läsas rekursivt från undermapparna eller endast från den angivna mappen. När rekursivt är inställt på True och sinken är en filbaserad lagring, kopieras inte en tom mapp eller undermapp till mottagaren. <br>Tillåtna värden är _true * (standard) och *falskt*.<br>Den här egenskapen gäller inte när du konfigurerar `fileListPath` . |Nej |
