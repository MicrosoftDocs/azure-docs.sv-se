---
title: 'Snabb start: Använd Java och JDBC med Azure Database for PostgreSQL flexibel Server'
description: I den här snabb starten får du lära dig hur du använder Java och JDBC med en Azure Database for PostgreSQL flexibel Server.
author: mksuni
ms.author: sumuth
...
ms.locfileid: "98606128"
---
# <a name="quickstart-use-java-and-jdbc-with-azure-database-for-postgresql-flexible-server"></a>Snabb start: Använd Java och JDBC med Azure Database for PostgreSQL flexibel Server

Det här avsnittet visar hur du skapar ett exempel program som använder Java och [JDBC](https://en.wikipedia.org/wiki/Java_Database_Connectivity) för att lagra och hämta information i [Azure Database for PostgreSQL flexibel Server](./index.yml).
...
> [!NOTE]
> Du kan läsa mer detaljerad information om hur du skapar PostgreSQL-servrar i [skapa en Azure Database for postgresql-server med hjälp av Azure Portal](./quickstart-create-server-portal.md).
...
PostgreSQL-servern som du skapade tidigare har en tom databas- **postgres** . I den här snabb starten ska vi skapa en ny databas som kallas `demo` följande steg:

1. Installera en lokal instans av [psql](https://www.postgresql.org/docs/current/static/app-psql.html) för att ansluta till en Azure postgresql-server. 
2. Kör följande psql-kommando för att ansluta till **postgres** -databasen på postgresql-servern
...
3. Skapa en ny databas med namnet **demo** i frågan genom att skriva följande kommando:
