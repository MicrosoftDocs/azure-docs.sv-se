...
När en klient skickar ett meddelande vill det vanligt vis veta om meddelandet har överförts korrekt till och godkänts av Service Broker eller om någon sorts fel uppstod. Med den här positiva eller negativa bekräftelsen kvittas klienten och Service Broker-överenskommelsen om överförings tillstånd för meddelandet. Så kallas det för *kvittning*.
...
Strategin för att hantera resultatet av sändnings åtgärder kan ha omedelbar och betydande prestanda påverkan för ditt program. Exemplen i det här avsnittet är skrivna i C# och gäller för Java-framtiden, Java-svartviter, JavaScript-löfte och motsvarande koncept på andra språk.
...
Det är viktigt att Observera att alla asynkrona programmerings modeller använder någon form av minnesallokering-baserad, Dold arbetskö som innehåller väntande åtgärder. När sändnings-API: n returnerar, köas sändnings uppgiften i den arbets kön, men protokoll-gesten börjar bara när det är så att uppgiften börjar köras. För kod som är försiktig för att skicka meddelanden till och var tillförlitlighet är ett problem, bör du tänka på att inte för många meddelanden placeras i flygningen på samma gång, eftersom alla skickade meddelanden tar upp minnet tills de faktiskt har placerats i kabeln.
...
Läget för att **ta emot och ta bort** instruerar koordinatorn att ta hänsyn till alla meddelanden som skickas till den mottagande klienten som kvittas när de skickas. Det innebär att meddelandet anses förbrukat så snart som Service Broker har placerat det på kabeln. Om meddelande överföringen Miss lyckas går meddelandet förlorat.
...
Läget **Peek-lock** visar att den mottagande klienten vill lösa mottagna meddelanden explicit. Meddelandet görs tillgängligt för att mottagaren ska kunna bearbeta, medan den hålls under ett exklusivt lås i tjänsten så att andra konkurrerande mottagare inte kan se det. Längden på låset definieras ursprungligen på kö-eller prenumerations nivå och kan utökas av klienten som äger låset. Mer information om hur du förnyar lås finns i avsnittet [förnya lås](#renew-locks) i den här artikeln. 
...
När meddelandet har frigjorts upprepade gånger av mottagare eller om de tillåter att låset förflyter under ett angivet antal gånger ([maximalt antal leveranser](service-bus-dead-letter-queues.md#maximum-delivery-count)) tas meddelandet bort automatiskt från kön eller prenumerationen och placeras i den associerade kön för obeställbara meddelanden.

Den mottagande klienten initierar kvittning av ett mottaget meddelande med en positiv bekräftelse vid anrop till `Complete` API: et för meddelandet. Den anger att Service Broker har bearbetats och att meddelandet tas bort från kön eller prenumerationen. Service Broker svarar på mottagarens kvittnings avsikt med ett svar som anger om kvittningen kan utföras.

När den mottagande klienten Miss lyckas med att bearbeta ett meddelande, men vill att meddelandet ska skickas igen, kan du uttryckligen be att meddelandet släpps och låses upp direkt genom att anropa `Abandon` API: et för meddelandet eller så kan det göra att det inte går att låsa.

Om en mottagar klient inte kan bearbeta ett meddelande och vet att det inte går att komma åt meddelandet och försöka utföra åtgärden igen, kan det avvisa meddelandet, som flyttar det till kön för obeställbara meddelanden genom att anropa API: `DeadLetter` et i meddelandet, vilket innebär också att du kan ställa in en anpassad egenskap inklusive en orsaks kod som kan hämtas med meddelandet från kön för obeställbara meddelanden.

Ett specialfall av kvittning är uppskjutande, som beskrivs i en [separat artikel](message-deferral.md).

`Complete`Åtgärderna, `Deadletter` eller `RenewLock` kan Miss lyckas på grund av nätverks problem, om det kvarhållna låset har upphört att gälla eller om det finns andra villkor för service sidan som förhindrar kvittning. I ett av de sistnämnda fallen skickar tjänsten en negativ bekräftelse på att det finns ett undantag i API-klienterna. Om orsaken är en bruten nätverks anslutning, släpps låset eftersom Service Bus inte stöder återställning av befintliga AMQP-länkar på en annan anslutning.

Om detta `Complete` Miss lyckas, som vanligt vis sker i mycket av meddelande hanteringen och i vissa fall efter bearbetning av antalet minuter, kan mottagar programmet avgöra om det bevarar det aktuella arbetet och ignorerar samma meddelande när det levereras en andra gång, eller om det tosses arbets resultatet och försöker igen när meddelandet har levererats igen.
...
## <a name="renew-locks"></a>Förnya lås
Standardvärdet för lås varaktigheten är **30 sekunder**. Du kan ange ett annat värde för lås varaktigheten på kö-eller prenumerations nivå. Klienten som äger låset kan förnya meddelande låset genom att använda metoder på objektet mottagare. I stället kan du använda funktionen för automatisk lås förnyelse där du kan ange hur länge du vill hålla låset förnyat. 

## <a name="next-steps"></a>Nästa steg
- Ett specialfall av kvittning är ett avstängnings fall. Mer information finns i [meddelande uppskjutaren](message-deferral.md) . 
- Information om obeställbara meddelanden finns i [köer för obeställbara meddelanden](service-bus-dead-letter-queues.md).
- Mer information om Service Bus meddelanden i allmänhet finns i [Service Bus köer, ämnen och prenumerationer](service-bus-queues-topics-subscriptions.md)