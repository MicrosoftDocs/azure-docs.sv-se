...
> [!NOTE]
> [Den här artikeln](virtual-nodes.md) ger dig en översikt över regionens tillgänglighet och begränsningar med hjälp av virtuella noder.
...
Om du inte tidigare har använt ACI registrerar du tjänstleverantören med din prenumeration. Du kan kontrollera statusen för ACI-providerregistreringen med kommandot [az provider list,][az-provider-list] som du ser i följande exempel:

```azurecli-interactive
az provider list --query "[?contains(namespace,'Microsoft.ContainerInstance')]" -o table
```

Providern *Microsoft.ContainerInstance* ska rapportera som *Registrerad*, enligt följande exempelutdata:
...
Om providern visas som *NotRegistered* registrerar du providern med hjälp av [az provider-registret][az-provider-register] enligt följande exempel:
...
- *PRIMÄR NODPOOL:* Välj en VM-storlek för AKS-noderna. VM-storleken **kan inte** ändras efter att ett AKS-kluster har distribuerats.
     - Välj även det antal noder som ska distribueras till klustret. I den här artikeln anger du **Nodantal** till *1*. Antalet noder **kan** justeras efter att klustret har distribuerats.
...
Klustret är också konfigurerat för avancerade nätverk. De virtuella noderna är konfigurerade för att använda sitt eget virtuella Azure-nätverksundernät. Det här undernätet har delegerad behörighet att ansluta Azure-resurser mellan AKS-klustret. Om du inte redan har delegerat undernät skapar och konfigurerar Azure Portal azure-nätverket och undernätet för användning med de virtuella noderna.
...
Följande exempelutdata visar den enskilda VM-nod som skapats och sedan den virtuella noden för Linux, *virtual-node-aci-linux:*
...
Nu får du åtkomst till adressen för podden `curl` med , till exempel *http://10.241.0.4* . Ange din egen interna IP-adress som visas i föregående `kubectl get pods` kommando:
...
Stäng terminalsessionen till testpodden med `exit` . När sessionen avslutas tas podden bort.
