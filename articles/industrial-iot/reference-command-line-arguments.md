---
title: Kommando rads argument för Microsoft OPC Publisher
description: Den här artikeln innehåller en översikt över kommando rads argumenten för OPC-utgivaren
author: jehona-m
ms.author: jemorina
...
ms.locfileid: "104787594"
---
# <a name="command-line-arguments"></a>Kommando rads argument

I följande finns flera kommando rads argument som beskrivs som kan användas för att ange globala inställningar för OPC-utgivare.

## <a name="opc-publisher-command-line-arguments-for-version-25-and-below"></a>Kommando rads argument för OPC Publisher för version 2,5 och senare

* Användning: opcpublisher.exe \<applicationname> [ \<iothubconnectionstring> ] [ \<options> ]
    
* ApplicationName: det OPC UA-programnamn som ska användas, vilket krävs för att program namnet även ska användas för att registrera utgivaren under detta namn i IoT Hub enhets registret.
    
* iothubconnectionstring: den IoT Hub ägarens ConnectionString, valfritt. Vanligt vis anger du endast IoTHub-ägarens ConnectionString i den första starten av programmet. Anslutnings strängen krypteras och lagras i plattformens certifikat arkiv.
På efterföljande anrop läses de från där och återanvänds. Om du anger ConnectionString för varje start tas enheten, som skapas för programmet i IoT Hub enhets registret, bort och återskapas varje tillfälle.
    
Det finns ett par miljövariabler som kan användas för att styra programmet:
```
...
> [!NOTE] 
> Kommando rads argument åsidosätter miljö variabel inställningar.
...
## <a name="opc-publisher-command-line-arguments-for-version-26-and-above"></a>Kommando rads argument för OPC Publisher för version 2,6 och senare
```
      --pf, --publishfile=VALUE
                                       the filename to configure the nodes  to publish.
...
Du hittar ytterligare resurser i GitHub-databaserna:
