...
- **Utgående anspråk**: anspråk som genereras av Azure-attestering och ingår i attesterings-token
...
Anspråk som ska användas av princip författare för att definiera auktoriseringsregler i en princip för SGX-attestering:

- **x-MS-SGX-är-fel sökning**: ett booleskt värde som anger om enklaven har fel sökning aktiverat eller inte
- **x-MS-SGX-Product-ID**: produkt-ID-värde för SGX-enklaven 
...
Anspråk som ska användas av princip författare för att definiera auktoriseringsregler i en princip för TPM-attestering:
...
Förutom policyn för TPM-attestering kan du använda anspråk nedan för att definiera auktoriseringsregler i en princip författare.
...
### <a name="common-for-all-attestation-types"></a>Gemensamt för alla typer av attestering

Azure-attestering innehåller nedanstående anspråk i attesterings-token för alla attesterings typer. 
...
- **x-MS-policy-Signer**: JSON-objekt med en "JwK"-medlem som representerar den nyckel en kund använde för att signera sin princip. Detta gäller när kunden laddar upp en signerad princip

Under anspråks namn används från [IETF JWT-specifikationen](https://tools.ietf.org/html/rfc7519)

- **"JTI" (JWT ID)-anspråk** – unik IDENTIFIERARE för JWT
- **"ISS" (Issuer)-anspråk** – huvud kontot som UTFÄRDAde JWT 
- **"IAT" (utfärdat vid)-anspråk** – tiden då JWT utfärdades 
- **"EXP" (förfallo tid) anspråk** -förfallo tid efter vilken JWT inte får godkännas för bearbetning
- **"NBF" (inte före)-anspråk** -inte före den tid som JWT inte får godkännas för bearbetning 

Under anspråks namn används från [IETF äta Draft-specifikation](https://tools.ietf.org/html/draft-ietf-rats-eat-03#page-9)

- **"Nonce-anspråk" (nonce)** – en icke-transformerad direkt kopia av ett valfritt nonce-värde som tillhandahålls av en klient 
...
- **x-MS-SGX-släktingar**: JSON-objekt som beskriver de säkerheter som används för att utföra attestering. Värdet för anspråket x-MS-SGX-marknadsföring är ett kapslat JSON-objekt med följande nyckel/värde-par:
    - **qeidcertshash**: SHA256 värde för-ID (quoting enklaven) som utfärdar certifikat
    ...
    - **tcbinfohash**: SHA256-värde för TCB info-material
...
### <a name="tpm-and-vbs-attestation"></a>TPM-och VBS-attestering
...
- **report_validity_in_minutes**: ett heltals anspråk för att tyda på hur länge token är giltig.
