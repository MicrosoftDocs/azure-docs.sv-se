...
Som standard återspeglar tjänsttaggar intervallen för hela molnet. Vissa tjänsttaggar tillåter också mer detaljerad kontroll genom att begränsa motsvarande IP-intervall till en angiven region. Till exempel representerar tjänsttaggen **Storage** Azure Storage för hela molnet, men **Storage.WestUS** begränsar intervallet till endast lagrings-IP-adressintervallen från regionen WestUS. I följande tabell visas om varje tjänsttagg stöder ett sådant regionalt omfång.  
...
| **ApiManagement** | Hanteringstrafik för Azure API Management-dedikerade distributioner. <br/><br/>*Obs!* Den här taggen representerar Azure API Management-tjänstslutpunkten för kontrollplanet per region. Detta gör att kunder kan utföra hanteringsåtgärder på API:er, åtgärder, principer, NamedValues som konfigurerats på API Management tjänsten.  | Inkommande | Ja | Ja |
...
| **AzureAPIForFHIR** | Azure API for FHIR (FHIR (elektronisk standard för utbyte av vårdinformation)).<br/><br/> *Obs! Den här taggen kan för närvarande inte konfigureras via Azure Portal.*| Utgående | Inga | Inga |
| **AzureArcInfrastructure** | Azure Arc-aktiverade servrar, Azure Arc aktiverat Kubernetes och gästkonfigurationstrafik.<br/><br/>*Obs!* Den här taggen är beroende av **taggarna AzureActiveDirectory,****AzureTrafficManager** och **AzureResourceManager.** *Den här taggen kan för närvarande inte konfigureras via Azure Portal*.| Utgående | Inga | Ja |
...
| **AzureDevOps** | Azure Dev Ops.<br/><br/>*Obs! Den här taggen kan för närvarande inte konfigureras via Azure-portalen*| Inkommande | Inga | Ja |
...
| **AzureMonitor** | Log Analytics, Application Insights, AzMon och anpassade mått (GiG-slutpunkter).<br/><br/>*Obs!* För Log Analytics krävs även lagringstaggen.  Om Linux-agenter används krävs **taggen GuestAndHybridManagement.** | Utgående | Inga | Ja |
...
> [!NOTE]
> Det tar upp till 4 veckor för nya tjänsttaggdata att spridas i API-resultaten. Ändringsnumret i svarsmetadata ökas när detta sker. Det kan finnas tillfälliga skillnader i resultat när olika platsvärden anges. När du använder resultaten för att skapa NSG-regler bör du ange platsparamatern så att den matchar NSG:ns region. 
...
- Exempel på hur du parsar information om tjänsttaggar (till exempel hämta alla adressintervall för Storage i USA, västra) finns i PowerShell-dokumentationen för API:et för [identifiering av tjänsttaggar.](/powershell/module/az.network/Get-AzNetworkServiceTag)
- När nya IP-adresser läggs till i tjänsttaggar används de inte i Azure under minst en vecka. Detta ger dig tid att uppdatera alla system som kan behöva spåra DE IP-adresser som är associerade med tjänsttaggar.
