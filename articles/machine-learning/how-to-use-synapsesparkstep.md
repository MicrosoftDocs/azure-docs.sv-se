...
description: Länka din Synapse-arbetsyta till din Azure Machine Learning-pipeline för att använda Spark för data manipulering.
...
ms.locfileid: "101663992"
---
# <a name="how-to-use-apache-spark-in-your-machine-learning-pipeline-with-azure-synapse-preview"></a>Använda Apache Spark i din Machine Learning-pipeline med Azure Synapse (för hands version)

I den här artikeln får du lära dig hur du använder Apache Spark pooler som stöds av Synapse som beräknings mål för ett steg för förberedelse av data i en Azure Machine Learning pipeline. Du får lära dig hur en enskild pipeline kan använda beräknings resurser som passar för det aktuella steget, till exempel data förberedelse eller utbildning. Du ser hur data förbereds för Spark-steget och hur de skickas till nästa steg. 
...
* Skapa en Synapse-arbetsyta och Apache Spark pool (se [snabb start: skapa en server lös Apache Spark pool med Synapse Studio](../synapse-analytics/quickstart-create-apache-spark-pool-studio.md)). 

## <a name="link-your-machine-learning-workspace-and-synapse-workspace"></a>Länka din Machine Learning-arbetsyta och Synapse-arbetsyta 

Du skapar och administrerar Apache Spark pooler på en Synapse-arbetsyta. Om du vill integrera en spark-pool med en Azure Machine Learning arbets yta måste du länka till arbets ytan Synapse. 

Du kan ansluta en Synapse Spark-pool via Azure Machine Learning Studio-ANVÄNDARGRÄNSSNITTET med hjälp av sidan **länkade tjänster** . Du kan också göra det via **beräknings** sidan med alternativet **bifoga beräkning** .

Du kan också bifoga en Synapse Spark-pool via SDK (enligt nedan) eller via en ARM-mall (se den här [exempel arm-mallen](https://github.com/Azure/azure-quickstart-templates/blob/master/101-machine-learning-linkedservice-create/azuredeploy.json)). 

Du kan använda kommando raden för att följa ARM-mallen, lägga till den länkade tjänsten och bifoga Synapse-poolen med följande kod:
...
> [!Important]
> Om du vill länka till Synapse-arbetsytan måste du ha ägar rollen i Synapse-arbetsytans resurs. Kontrol lera åtkomsten i Azure Portal.
...
## <a name="create-or-retrieve-the-link-between-your-synapse-workspace-and-your-azure-machine-learning-workspace"></a>Skapa eller Hämta länken mellan din Synapse-arbetsyta och din Azure Machine Learning-arbetsyta
...
## <a name="attach-your-synapse-spark-pool-as-a-compute-target-for-azure-machine-learning"></a>Anslut Synapse Spark-poolen som ett beräknings mål för Azure Machine Learning

Om du vill använda Synapse Spark-poolen för att sätta ett steg i din Machine Learning-pipeline måste du koppla den som en `ComputeTarget` för pipeline-steget, som du ser i följande kod.
...
Förutom data kan ett pipeline-steg ha svar på python-beroenden per steg. Enskilda `SynapseSparkStep` objekt kan också ange den exakta Synapse-konfigurationen. Detta visas i följande kod, som anger att `azureml-core` paket versionen måste vara minst `1.20.0` . (Som tidigare nämnts krävs detta krav för `azureml-core` att använda en `FileDataset` som inmatad.)
...
Nästa uppsättning argument för `SynapseSparkStep` konstruktorn som styr Apache Spark. `compute_target`Är det `'link1-spark01'` som vi kopplade till som ett beräknings mål tidigare. De andra parametrarna anger vilka minnen och kärnor som vi vill använda.
