...
När det automatiserade ML-experimentet har slutförts kan du hitta en historik över körningarna via:
  ...
  - En Jupyter Notebook med [Jupyter-widgeten RunDetails](/python/api/azureml-widgets/azureml.widgets.rundetails)
...
Många klassificeringsmått definieras för binär klassificering på två klasser och kräver medelvärde över klasser för att producera en poäng för klassificering med flera klasser. Scikit-learn tillhandahåller flera medelvärdesmetoder, varav tre automatiserade ML exponerar: **makro,** **mikro** och **viktad**.
...
- **Viktad** – Beräkna måttet för varje klass och ta det viktade medelvärdet baserat på antalet urval per klass.

Även om varje medelvärdesmetod har sina fördelar, är en vanlig faktor när du väljer lämplig metod obalans i klassen. Om klasserna har olika antal urval kan det vara mer informativt att använda ett makrogenomsnitt där klasser för klasser med olika klasser får samma viktning som majoritetsklasserna. Läs mer om [binära och flerklassmått i automatiserad ML.](#binary-vs-multiclass-classification-metrics) 

I följande tabell sammanfattas de modellprestandamått som automatiserad ML beräknar för varje klassificeringsmodell som genererats för experimentet. Mer information finns i scikit-learn-dokumentationen som är länkad i **beräkningsfältet** för varje mått. 

|Metric|Beskrivning|Beräkning|
|--|--|---|
|AUC | AUC är området under kurva för [mottagaroperativsystemet.](#roc-curve)<br><br> **Mål:** Närmare 1 desto bättre <br> **Intervall:** [0, 1]<br> <br>Måttnamn som stöds är: <li>`AUC_macro`, det aritmetiska medelvärdet för AUC för varje klass.<li> `AUC_micro`, beräknat genom att kombinera sanna positiva och falska positiva resultat från varje klass. <li> `AUC_weighted`, det aritmetiska medelvärdet av poängen för varje klass, viktat med antalet sanna instanser i varje klass.<br><br>Obs! AUC-värdena som rapporteras av automatiserad ML kanske inte matchar ROC-diagrammet om det bara finns två klasser. För binär klassificering tillämpar den underliggande scikit-learn-implementeringen av AUC inte makro-/mikro-/viktat medelvärde. I stället returneras AUC för den mest sannolika positiva klassen. ROC-diagrammet fortsätter att använda ett medelvärde för klassen för binär klassificering precis som det gör för flera klasser.  |[Beräkning](https://scikit-learn.org/stable/modules/generated/sklearn.metrics.roc_auc_score.html) | 
...
|average_precision|Genomsnittlig precision sammanfattar en kurva för precisions träffsäkerhet som det viktade medelvärdet av precisioner som uppnås vid varje tröskelvärde, med en ökning av träffsäkerhet från föregående tröskelvärde som används som viktning. <br><br> **Mål:** Närmare 1 desto bättre <br> **Intervall:** [0, 1]<br> <br>Måttnamn som stöds är:<li>`average_precision_score_macro`, det aritmetiska medelvärdet av den genomsnittliga precisionspoängen för varje klass.<li> `average_precision_score_micro`, beräknat genom att kombinera sanna positiva och falska positiva resultat vid varje cutoff.<li>`average_precision_score_weighted`, det aritmetiska medelvärdet av den genomsnittliga precisionspoängen för varje klass, viktat med antalet sanna instanser i varje klass.|[Beräkning](https://scikit-learn.org/stable/modules/generated/sklearn.metrics.average_precision_score.html)|
