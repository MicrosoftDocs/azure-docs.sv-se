---
title: Komplexa typer Service Fabric XML-schema för Azure Service Fabric-tjänstmodell
description: Beskriver de komplexa typerna i XML-schemat för Service Fabric tjänstmodellen.
ms.topic: reference
ms.date: 12/10/2018
...
Beskriver principen för utvärdering av hälsohändelser som rapporteras på olika programrelaterade entiteter. Om ingen princip anges antas en entitet vara skadad om hälsorapporten är en varning eller ett fel.
...
Anger den högsta procentandelen av distribuerade program som kan vara felaktiga innan programmet betraktas som ett fel. Detta beräknas genom att dividera antalet program med feltillstånd över antalet noder som programmen för närvarande distribueras på i klustret. Beräkningen avrundar uppåt för att tolerera ett fel på ett litet antal noder. Standardprocent: 0.
...
Anger standardhälsoprincipen för tjänsttyper, som ersätter standardhälsoprincipen för alla tjänsttyper i programmet.
...
Beskriver principen för utvärdering av hälsohändelser som rapporteras för tjänster, partitioner och repliker av en viss tjänsttyp.
...
Deklarativt beskriver programtyp och version. Ett eller flera tjänstmanifest för de ingående tjänsterna refereras till för att skapa en programtyp. Konfigurationsinställningarna för de ingående tjänsterna kan åsidosättas med hjälp av parametriserade programinställningar. Standardtjänster, tjänstmallar, huvudnamn, principer, diagnostikkonfiguration och certifikat kan också deklareras på programnivå.
...
#### <a name="parameters"></a>Parametrar
Deklarerar de parametrar som används i det här applikationsmanifestet. Värdet för dessa parametrar kan anges när programmet instansieras och kan användas för att åsidosätta konfigurationsinställningar för program eller tjänster.
...
Importerar ett tjänstmanifest som skapats av tjänstutvecklaren. Ett tjänstmanifest måste importeras för varje komponenttjänst i programmet. Åsidosättningar och principer för konfiguration kan deklareras för tjänstmanifestet.
...
ApplicationPackage representerar den versionsinformation för program som krävs av noden.
...
Beskriver de principer (logginsamling, standardåtkomst för kör som, hälsa och säkerhet) som ska tillämpas på programnivå.
...
Anger om logginsamling är aktiverad. Fungerar endast i en Azure-klustermiljö
...
Beskriver ett Azure Blob Store-mål för ETW-händelser. Fungerar endast i Azure-miljö.
...
Beskriver ett diagnostikarkiv i ett Azure Storage-konto.
...
#### <a name="fabricsettings"></a>Infrastrukturinställningar

|Attribut|Värde|
|---|---|
|name|Infrastrukturinställningar|
...
Anger om innehållet i det här kodpaketet delas av andra kodpaket. Om det är sant startas alla kodpaket om vid en uppgradering av det här kodpaketet. Det här attributet stöds inte för närvarande och värdet ignoreras.
...
Beskriver åsidosättningar av konfiguration för ett visst konfigurationspaket i det importerade tjänstmanifestet.
...
Avsnittsnamn för ConfigPackage-Settings.xml.
...
Deklarerar en mapp med namnet med attributet Name under PackageRoot som innehåller en Settings.xml fil. Den här filen innehåller avsnitt med användardefinierade nyckel/värde-parinställningar som processen kan läsa tillbaka vid körning. Om bara ConfigPackage-versionen har ändrats under en uppgradering startas inte den process som körs om. I stället meddelar ett återanrop processen att konfigurationsinställningarna har ändrats så att de kan läsas in dynamiskt på nytt.
...
Anger information om ett X509-certifikat som ska exponeras för containermiljön. Certifikatet måste installeras i LocalMachine-arkivet för alla klusternoder.
...
PFX-filen och lösenordet är tillgängliga i containern med hjälp av Certificates_ServicePackageName_CodePackageName_CertName_PFX och Certificates_ServicePackageName_CodePackageName_CertName_Password miljövariabler. PEM-filen är tillgänglig i containern med hjälp av Certificates_ServicePackageName_CodePackageName_CertName_PEM och Certificates_ServicePackageName_CodePackageName_CertName_PrivateKey miljövariabler.
...
Namnet på det datapaket som innehåller certifikatfilerna.
...
Den version av datapaketet som innehåller certifikatfilerna.
...
Om sant krypteras värdet för lösenordet.
...
Om containern har HEALTHCHECK aktiverat och Docker rapporterar health_status händelse för den här containern Service Fabric detta som en del av systemets rapporterade hälsa. När health_status är skadad Service Fabric en hälsovarning. Som standard är det inställt på sant.
...
Om containern har HEALTHCHECK aktiverat och Docker rapporterar health_status händelse för den här containern startar Service Fabric om containern när health_status av Docker inte är fel.
Som standard är det inställt på falskt.
...
Lagringsplatsen och https://hub.docker.com avbildningen på eller Azure Container Registry.
...
Isoleringsläge för container. Giltiga värden är standard, process eller Hyper-V (stöds endast för Windows-containrar).
...
Antal containrar som ska vara kvar när containern upprepade gånger har förts bort. Som standard är detta inställt på 0, så inga containrar lämnas kvar.
...
Flagga som anger om borttagning av containrar sker automatiskt (sant) eller inte (falskt) om containern tas bort.  Om false SF tas bort manuellt tar du bort containrarna.

|Attribut|Värde|
|---|---|
|name|Ta bort automatiskt|
|använda|valfri|
|typ|xs:string|

#### <a name="runinteractive"></a>RunInteractive
Kör containern med interaktiv flagga aktiverad. Giltiga värden är true/false. falskt som standard.
...
Autentiseringsuppgifter för lagringsplatsen för containeravbildningar som avbildningar ska kunna hämtas från.
...
Anger information för ett certifikat som exponeras för containern.
...
Anger säkerhetalternativ för containern.
...
NetworkType. Typer som stöds för närvarande är "Öppna" och "Isolerad".
...
Beskriver associationen mellan ett tjänstpaket och ett containernätverk och eventuellt associationen mellan slutpunkterna för tjänstpaketet och containernätverket.
...
Namnet på containernätverket (icke-känsligt). Det måste vara namnet på ett containernätverk som redan har skapats eller reserverade namn, inklusive "Öppna" eller "NAT".
...
Källmappen som kan vara en mapp på den virtuella dator som är värd för containrarna eller ett beständigt fjärrarkiv.
...
Den plats som källan mappas till i containern som körs. Målet kan därför inte vara en plats som redan finns i containern.
...
#### <a name="driver"></a>Drivrutinen
Drivrutinsnamnet för plugin Azure Files volymen är "sfazurefile".
...
Deklarerar en mapp med namnet med attributet Name under PackageRoot som innehåller statiska datafiler som ska användas av processen vid körning. Service Fabric återanvänder alla EXE:er och DLLHOST:er som anges i värd- och supportpaketen när något av de datapaket som anges i tjänstmanifestet uppgraderas.
...
Anger information om felsökningsprogrammet som ska bifogas när du aktiverar codepackage.
...
Åsidosätt startpunkten för containrar så att felsökningsprogrammet kan startas.
...
Etiketter för containrar i form av key=value.
...
Deklarerar tjänstinstanser som skapas automatiskt när ett program instantieras mot den här programtypen.
...
En samling tjänster som automatiskt placeras tillsammans, så att de också flyttas tillsammans under redundans- eller resurshantering.
...
Anger kraschdumpsamling. Kraschdumpar samlas in för körbara filer som är värdar för kodpaketen för alla tjänster som hör till programmet.
...
Anger ETW-spårningssamling. ETW-spårningar samlas in för leverantörer som är registrerade av alla tjänster som hör till programmet.
...
Stöds inte, använd inte. STÖD för DLL-värd (sammansättningspostpunkt) tillhandahålls via FWP.exe processen. Service Fabric startar Fabric Worker-processen (FWP.exe) och läser in sammansättningen som en del av aktiveringsprocessen.
...
Stöds inte, använd inte. Definierar isoleringsprincipen för ohanterade DLL:er och hanterade sammansättningar som läses in i DllHost. 
...
#### <a name="unmanageddll"></a>OhanteradDll
...
Drivrutinsalternativ som ska skickas till drivrutinen. Plugin-programmet Azure Files volume stöder följande drivrutinsalternativ: shareName (Azure Files-filresursen som tillhandahåller volymen för containern), storageAccountName (Azure Storage-kontot som innehåller Azure Files-filresursen), storageAccountKey (åtkomstnyckel för Azure Storage-kontot som innehåller Azure Files-filresursen).
...
Det här värdet definierar skyddet av attributets värde. Det kan vara SecretsStoreRef/Encrypted/PlainText.
Om värdet är inställt på SecretsStoreRef refererar värdet till en hemlighet som lagras krypterad i klustrets SecureStore, som refereras till och dekrypteras vid aktivering.
Om det är inställt på Encrypted ansvarar programutvecklaren för att skapa ett certifikat och använda cmdleten Invoke-ServiceFabricEncryptSecret för att kryptera känslig information.
...
#### <a name="certificateref"></a>CertificateRef
Namnet på slutpunktscertifikatet, deklarerat i avsnittet Certifikat, för att återgå till klienten. 
...
#### <a name="name"></a>Name
Namnet på den slutpunkt som ska åsidosätta. En sträng.
...
Protokollet för slutpunkten. Åsidosätter värdet i tjänstmanifestet. HTTPS-slutpunkter måste också ha ett EndpointCertificate och en EndpointBindingPolicy deklarerad i programmanifestet. Protokollet kan inte ändras senare i en programuppgradering. 
...
#### <a name="type"></a>Typ
Typ av slutpunkt. Åsidosätter värdet i tjänstmanifestet. Indataslutpunkter används för att exponera porten utanför, och interna slutpunkter används för kommunikation mellan program.
...
Definierar en slutpunkt för tjänsten. Specifika portar kan begäras.  Om en port inte uttryckligen anges tilldelas en port från portintervallet för det reserverade programmet. Tjänstrepliker som körs på olika klusternoder kan tilldelas olika portnummer, medan repliker av samma tjänst som körs på samma nod delar samma port. Sådana portar kan användas av tjänstreplikerna för olika syften, till exempel replikering eller lyssna efter klientbegäranden.
...
#### <a name="protocol"></a>Protokoll
Protokollet för slutpunkten: http, https, tcp eller udp. HTTPS-slutpunkter måste också ha ett EndpointCertificate och en EndpointBindingPolicy deklarerad i programmanifestet. Protokollet kan inte ändras senare i en programuppgradering. 
...
#### <a name="type"></a>Typ
Typ av slutpunkt.  Indataslutpunkter används för att exponera porten utifrån, och interna slutpunkter används för kommunikation mellan program.
...
#### <a name="codepackageref"></a>CodePackageRef
Namnet på koden Paket som ska använda den här slutpunkten.
...
Den körbara fil som anges av EntryPoint är vanligtvis den långvariga tjänstvärden. Förekomsten av en separat startpunkt för installationen undviker att behöva köra tjänstvärden med hög behörighet under längre tidsperioder. Den körbara fil som anges av EntryPoint körs när SetupEntryPoint avslutas. Den resulterande processen övervakas och startas om (med början igen med SetupEntryPoint) om den avslutas eller kraschar.
...
Typ för miljövariabeln. Det kan vara PlainText/Encrypted/SecretsStoreRef.
Om det är inställt på SecretsStoreRef hämtar vi referensvärdet från SecretStore.
Om det är inställt på Encrypted ansvarar programutvecklaren för att skapa ett certifikat och använda cmdleten Invoke-ServiceFabricEncryptSecret för att kryptera känslig information.
...
Sant om den körbara filen är extern till programpaketet. Kontroll av att programmet finns i programpaketet hoppas över.
...
Varning! Använd inte konsolomdirigering i ett produktionsprogram, använd det bara för lokal utveckling och felsökning. Omdirigerar konsolens utdata från startskriptet till en utdatafil i programmappen med namnet "log" på klusternoden där programmet distribueras och körs.
...
Detta används endast när autentiseringssuppgifter är X509. Detta anger hur du hittar certifikatet, oavsett om det är med namnet eller tumavtrycket 
...
#### <a name="x509findvalue"></a>X509FindValue
Detta används endast när autentiseringssuppgifter är X509. Det här är det faktiska namnet eller tumavtrycket för certifikatet.
...
IP-adressen eller FQDN för den dator där den här noden ska placeras.
...
#### <a name="isseednode"></a>IsSeedNode
En flagga som anger om den här noden är en startnod eller inte.
...
#### <a name="nodetyperef"></a>NodeTypeRef
Namnet på den nodetype som definieras i avsnittet NodeTypes. 
...
Beskriver ett filarkivmål för ETW-händelser. Fungerar endast i en lokal miljö.
...
Windows Server-containrar kanske inte är kompatibla i olika versioner av operativsystemet.  Du kan ange flera OS-avbildningar per container och tagga dem med versionerna av operativsystemet. Hämta versionen av operativsystemet genom att köra "winver" i en Windows-kommandotolk. Om det underliggande operativsystemet är version 16299 (Windows Server version 1709) väljer Service Fabric containeravbildningen taggad med Os="16299". En otaggad containeravbildning antas fungera i alla versioner av operativsystemet och åsidosätter avbildningen som anges i tjänstmanifestet.
...
#### <a name="image"></a>Bild
Containeravbildning som motsvarar versionsnumret för operativsystemet som ska startas. Om attributet Os inte anges antas containeravbildningen fungera i alla versioner av operativsystemet och åsidosätter avbildningen som anges i tjänstmanifestet.
...
#### <a name="os"></a>Os
Operativsystemets build-version. Till exempel är build-versionen av Windows Server version 1709 16299.
...
Beskriver den infrastrukturinformation som behövs.
...
Namnet på rollen som länkar till nodtypen ref som definieras i avsnittet NodeTypes.
...
#### <a name="isseednode"></a>IsSeedNode
Anger om noden är en startpunktsnod.
...
Beskriva de slutpunkter som är associerade med den här nodtypen
...
En resurs som den här tjänsten ska balanseras på, till exempel minnes- eller CPU-användning.  Innehåller information om hur mycket av resursen som varje replik eller instans av den här tjänsten förbrukar som standard.
...
#### <a name="name"></a>Name
En unik identifierare för måttet i klustret Klusterresurshanteraren perspektivet.
...
Standardbelastningen som den här tjänsten använder för det här måttet när det är en primär replik.
...
Den standardmängd av belastning som den här tjänsten kommer att använda för det här måttet när det är en sekundär replik.
...
Avgör måttvikten i förhållande till de andra mått som har konfigurerats för den här tjänsten. Om två mått hamnar i konflikt under körningen föredrar Klusterresurshanteraren måttet med den högre vikten. Noll inaktiverar belastningsutjämning för det här måttet.
...
Beskriver ett lagringsmål i noden för ETW-händelser.
...
Beskriver nätverksprinciper, inklusive nätverksprinciper för containrar för tjänstpaketet.
...
Beskriver nätverksprinciper för containrar för tjänstpaketet.
...
Namnet på koden, konfigurationspaketet eller datapaketet som ska delas. Måste matcha namnet på paketet som definierats i tjänstmanifestet.
...
Det här värdet definierar den typ av lösenord som du har angett i attributet "Lösenord". Det kan vara SecretsStoreRef/Encrypted/PlainText.
Om det är inställt på SecretsStoreRef hämtar vi referensvärdet från SecretStore.
Om det är inställt på Encrypted ansvarar programutvecklaren för att skapa ett certifikat och använda cmdleten Invoke-ServiceFabricEncryptSecret för att kryptera känslig information.
...
Den mjuka gränsen (i MB) för minnesstyrning som endast tillämpas när minnesbegränsning har identifierats på noden. Måste vara ett positivt heltal.
...
Högsta I/O-hastighet (läsning och skrivning) när det gäller IOPS som kan användas. Måste vara ett positivt heltal.
...
Det maximala antalet I/O (byte per sekund) som kan användas (läsning och skrivning). Måste vara ett positivt heltal.
...
Gränser för kernelminne i byte.  Måste vara ett positivt heltal.  Observera att detta är Linux-specifikt och att Docker i Windows kommer att få felmeddelandet om detta har angetts.
...
Storleken på /dev/shm i byte. Om detta utelämnas använder systemet 64 MB.  Måste vara ett positivt heltal.  Observera att detta är Linux-specifikt, men docker kommer INTE att få felmeddelandet om det anges.  Den ignoreras helt enkelt.
...
Anger resursåsidosättningar för slutpunkter som deklarerats i tjänstmanifestresurser.
...
#### <a name="endpoints"></a>Slutpunkter
De tjänstslutpunkter som ska åsidosätts.
...
Anger den lokala användaren eller det lokala systemkontot som ett tjänstkodpaket ska köras som. Domänkonton stöds i Windows Server-distributioner där Azure Active Directory är tillgänglig. Som standard körs program under det konto som Fabric.exe körs under. Program kan också köras som andra konton, som måste deklareras i avsnittet Huvudkonton. Om du tillämpar en RunAs-princip på en tjänst och tjänstmanifestet deklarerar slutpunktsresurser med HTTP-protokollet måste du också ange en SecurityAccessPolicy för att se till att portarna som allokeras till dessa slutpunkter har korrekt åtkomstkontroll som anges för RunAs-användarkontot som tjänsten körs under. För en HTTPS-slutpunkt måste du också definiera en EndpointBindingPolicy för att ange namnet på certifikatet som ska returneras till klienten.
...
#### <a name="entrypointtype"></a>EntryPointType
Installationsprogrammet är den SetupEntryPoint som deklareras i tjänstmanifestet, den privilegierade startpunkten som körs före någon annan startpunkt.  Main är den EntryPoint som deklareras i tjänstmanifestet, vanligtvis den långvariga tjänstvärden. Alla är alla startpunkter.
...
En beskrivning av skalningsprincip som består av en utlösare och en mekanism för skalning. 
...
Ger åtkomstbehörighet till ett huvudnamn för en resurs (till exempel en slutpunkt) som definierats i ett tjänstmanifest. Normalt är det mycket användbart att kontrollera och begränsa åtkomsten för tjänster till olika resurser för att minimera säkerhetsrisker. Detta är särskilt viktigt när programmet har skapats från en samling tjänster från en marknadsplats som har utvecklats av olika utvecklare.
...
Den användare eller grupp som tilldelas åtkomstbehörighet till en resurs måste deklareras i avsnittet Huvudnamn.
...
Behörighet att bevilja: Läsa, Ändra eller Fullständig. Standardvärdet är Läsa.
...
Typen av resurs, som definieras i avsnittet Resurser i tjänstmanifestet. Slutpunkt eller certifikat.
...
Deklarerar en uppsättning grupper som säkerhetsobjekt, som kan refereras till i principer. Grupper är användbara om det finns flera användare för olika tjänst startpunkter och de måste ha vissa vanliga behörigheter som är tillgängliga på gruppnivå.
...
Anger om specifik information om Fabric Runtime ska exponeras för kodpaketen i tjänstpaketet.
...
Anger om Service Fabric replikerat blocklager ska användas för att tillhandahålla volym för containrar i tjänstpaketet.
...
#### <a name="name"></a>Name
Namnet på tjänstgruppmedlemmen i förhållande till namnet på tjänstgruppen.
...
Bastyp som beskriver en tillståndslös eller tillståndslös ServiceGroupType.
...
Användardefinierad typidentifierare för en tjänstgrupp, till exempel "ActorQueueSGType". Det här värdet används i ApplicationManifest.xml för att identifiera tjänstgruppen.
...
Anger om tjänstgruppinstansen skapas av den implicita fabriken. Om det är falskt (standard) måste ett av kodpaketen registrera tjänstgruppfabriken
...
Medlemstyper för den här tjänstgrupptypen.
...
Beskriver principer (slutpunktsbindning, paketdelning, kör som-åtkomst och säkerhetsåtkomst) som ska tillämpas på det importerade tjänstmanifestet.
...
Anger principer som bestämmer hur Service Fabric-körningen exponeras för replikerna.
...
Anger principer för att aktivera containervärdar.
...
Anger nätverksprinciper inklusive principer för containernätverk.
...
Importerar tjänstmanifestet enligt referens. För närvarande måste tjänstmanifestfilen (ServiceManifest.xml) finnas i build-paketet.
...
Deklarativt beskriver tjänsttyp och version. Den visar en lista över oberoende uppgraderingsbara kod-, konfigurations- och datapaket som tillsammans utgör ett tjänstpaket för att stödja en eller flera tjänsttyper. Resurser, diagnostikinställningar och tjänstmetadata, till exempel tjänsttyp, hälsoegenskaper och mått för belastningsutjämning, anges också.
...
Definierar vilka tjänsttyper som stöds av en CodePackage i det här manifestet. När en tjänst instansieras mot en av dessa tjänsttyper aktiveras alla kodpaket som deklareras i det här manifestet genom att deras startpunkter körs. Tjänsttyper deklareras på manifestnivå och inte på kodpaketnivå.
...
#### <a name="isolation"></a>Isolering
Isoleringsläge för container. Giltiga värden är "standard", "process" eller "hyperv".
...
#### <a name="portbinding"></a>PortBinding
Anger vilken slutpunktsresurs som ska binda den exponerade containerporten.
...
ServicePackage representerar en versionsenhet för distribution och aktivering. Versionen av ServicePackage bestäms baserat på manifestversionen och åsidosättningens version.
...
#### <a name="manifestchecksum"></a>ManifestChecksumma
Checksum-värdet för ServiceManifest-filen.

|Attribut|Värde|
|---|---|
|name|ManifestChecksumma|
|typ|xs:string|

#### <a name="contentchecksum"></a>ContentChecksum
Checksum-värdet för det här ServicePackage-innehållet.
...
#### <a name="servicetypename"></a>ServiceTypeName
Namnet på tjänsttypen som deklareras i tjänstmanifestet och som instansieras.
...
Anger standardkostnad för flytt för den här tjänsten.
...
#### <a name="loadmetrics"></a>LoadMetrics
Belastningsmått som rapporteras av den här tjänsten används för resursbalanseringstjänster.
...
Deklarerar taggar som krävs för att placera en tjänst. Användbart för att dynamiskt påverka tjänstens placering. 
...
Deklarerar taggar som krävs för placering och körning av en tjänst. Användbart för att dynamiskt påverka tjänstens placering och körning. 
...
Definierar egenskaper för persistence- och avlägsningsprinciperna.
...
Anger den högsta procentandelen tjänster som inte är felhälsosamma innan programmet anses vara felaktigt. Standardprocent: 0.
...
Anger den högsta procentandelen av de felaktiga partitionerna innan en tjänst anses vara skadad. Standardprocent: 0.
...
Anger den högsta procentandelen av de felaktiga replikerna innan en partition anses vara skadad. Standardprocent: 0.
...
Bastyp som beskriver en tillståndslös eller tillståndslös ServiceType.
...
#### <a name="servicetypename"></a>ServiceTypeName
Användardefinierad typidentifierare för en tjänst. Till exempel "QueueType" eller "CalculatorType". Det här värdet används i ApplicationManifest.xml för att identifiera tjänsten.
...
#### <a name="statefulservicetype"></a>StatefulServiceType
Beskriver en tillståndsfull ServiceType.
...
## <a name="settingsoverridestype-complextype"></a>InställningarOverridesType complexType
Deklarerar konfigurationsinställningar i ett tjänstmanifest som ska åsidosättas. Den består av ett eller flera avsnitt av nyckel/värde-par. Parametervärden kan krypteras med hjälp Invoke-ServiceFabricEncryptSecret cmdlet.
...
Ett avsnitt i den Settings.xml som ska åsidosätts.
...
Definierar en tillståndsfull tjänstgrupp.
...
Beskriver en tillståndsfull tjänstgrupptyp.
...
Definierar en tillståndsfull tjänst.
...
Beskriver en tillståndsfull tjänsttyp.
...
Stöds inte, använd inte. Namnet på den ohanterade sammansättningen (till exempel Queue.dll) som värd.
...
#### <a name="nodename"></a>NodeName
Det här är namnet på noden som uppgraderingen av infrastruktur ska ske till
...
En flagga som anger om RemoveNodeState Api ska anropas när nodkonfigurationen har tas bort.
...
Namnet på den exe som används av installationstjänsten för att uppgradera 
...
Parametrar för installationens startpunkt exe
...
Namnet på den exe som används av installationstjänsten för att ångra uppgraderingen
