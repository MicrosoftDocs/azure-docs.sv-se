...
ms.locfileid: "107285347"
---
# <a name="azure-security-baseline-for-linux-virtual-machines"></a>Azures säkerhets bas linje för Virtuella Linux-datorer

Den här säkerhets bas linjen använder vägledning från [Azures säkerhets benchmark version 1.0](../../security/benchmarks/overview-v1.md) till virtuella Linux-datorer. Azure Security Benchmark ger rekommendationer om hur du kan skydda dina molnlösningar i Azure. Innehållet grupperas efter de **säkerhets kontroller** som definieras av Azures säkerhets benchmark och relaterade rikt linjer som gäller för virtuella Linux-datorer. 

> [!NOTE]
> **Kontroller** som inte är tillämpliga på virtuella Linux-datorer, eller för vilka ansvaret är Microsofts, har uteslutits. Om du vill se hur Virtuella Linux-datorer helt mappar till Azures säkerhets mätning, se den **[fullständiga virtuella Linux-datorer mappnings filen för säkerhets bas linjen](https://github.com/MicrosoftDocs/SecurityBenchmarks/raw/master/Azure%20Offer%20Security%20Baselines/1.1/linux-virtual-machines-security-baseline-v1.1.xlsx)**.
...
**Azure policy inbyggda definitioner – Microsoft. ClassicCompute**:
...
- [Om du vill avbilda syslog-data för övervakning måste du aktivera Log Analytics-tillägget](https://docs.microsoft.com/azure/azure-monitor/vm/quick-collect-azurevm#enable-the-log-analytics-vm-extension)
...
- [Översikt över Log Analytics-agenten](../../azure-monitor/agents/log-analytics-agent.md)
...
- [Översikt över Log Analytics-agenten](/azure/azure-monitor/platform/log-analytics-agent)
...
- [Följande länk innehåller Microsofts rekommenderade säkerhets rikt linjer, som kan fungera som en villkors lista för det sårbarhets program som valts](/azure/virtual-machines/linux/security-recommendations)
...
**Vägledning**: Virtuella Linux-datorer och Azure Active Directory (Azure AD) har inte begreppet standard lösen ord. Kund ansvarig för program från tredje part och Marketplace-tjänster som kan använda standard lösen ord.
...
**Vägledning**: skapa standard procedurer för användning av dedikerade administrativa konton som har åtkomst till dina virtuella datorer. Använd Azure Security Center identitets-och åtkomst hantering för att övervaka antalet administrativa konton. Alla administratörs konton som används för att komma åt resurser för virtuella Azure-datorer kan också hanteras av Azure Privileged Identity Management (PIM). Azure Privileged Identity Management tillhandahåller flera alternativ, till exempel bara i tids ökning, vilket kräver multifaktorautentisering innan du antar en roll och Delegerings alternativ så att behörigheter endast är tillgängliga för vissa tids ramar och kräver en god kännare.
...
**Vägledning**: där det är möjligt kan kunden använda SSO med Azure Active Directory (Azure AD) i stället för att konfigurera enskilda fristående autentiseringsuppgifter per tjänst. Använd rekommendationer för Azure Security Center identitets-och åtkomst hantering.
...
**Vägledning**: Använd Azure Active Directory (Azure AD) principer för villkorlig åtkomst och namngivna platser för att tillåta åtkomst från enbart vissa logiska grupperingar av IP-adressintervall eller länder/regioner.
...
**Vägledning**: Konfigurera diagnostikinställningar för Azure Active Directory (Azure AD) för att skicka gransknings loggar och inloggnings loggar till en Log Analytics arbets yta. Du kan också använda Azure Monitor för att granska loggar och utföra frågor om autentisering av syslog-data från virtuella Azure-datorer.
...
- [Microsofts rekommenderade säkerhets rikt linjer](/azure/virtual-machines/linux/security-recommendations)
...
- [Uppdateringshantering lösning i Azure](/azure/automation/overview)

- [Hantera uppdateringar och korrigeringar för dina virtuella Azure-datorer](/azure/automation/manage-updates-for-vm)
...
Använd dessutom Azure Resource Graph för att fråga/identifiera resurser i prenumerationerna. Detta kan hjälpa till med hög säkerhets-baserade miljöer, till exempel de med lagrings konton.
...
Förutom att använda Ändringsspårning för övervakning av program vara, kan anpassningsbara program kontroller i Azure Security Center använda Machine Learning för att analysera programmen som körs på datorerna och skapa en tillåten från den här intelligensen. Den här funktionen fören klar processen att konfigurera och underhålla principer för program tillåten, vilket gör att du kan undvika oönskad program vara som kan användas i din miljö. Du kan konfigurera gransknings läge eller framtvinga läge. Gransknings läget granskar endast aktiviteten på de skyddade virtuella datorerna. Tvingande läge tvingar fram reglerna och ser till att program som inte får köras blockeras.

- [Diagnostiskt tillägg för virtuella Linux-datorer](https://docs.microsoft.com/azure/virtual-machines/extensions/diagnostics-linux?toc=/azure/azure-monitor/toc.json)
...
**Vägledning**: Använd Azure Security Center rekommendation reparera sårbarheter i säkerhetskonfigurationer på din Virtual Machines för att underhålla säkerhetskonfigurationer för alla beräknings resurser.
...
- [Ladda upp en anpassad virtuell hård disk till Azure](../windows/upload-generalized-managed.md)
...
**Vägledning**: om du använder anpassade avbildningar (t. ex. virtuell hård disk) använder du Azure RBAC för att se till att endast behöriga användare kan komma åt avbildningarna.
...
- [Så här konfigurerar du Microsoft Antimalware för Cloud Services och Virtual Machines](/azure/virtual-machines/linux/security-recommendations)
...
**Vägledning**: Säkerställ att du regelbundet kan utföra Data återställning av innehåll inom Azure Backup. Om det behövs kan du prova att återställa innehållet till ett isolerat virtuellt nätverk eller en prenumeration. Kunden att testa återställning av säkerhetskopierade nycklar som hanteras av kunden.
