...
ms.locfileid: "104604244"
---
# <a name="azure-security-baseline-for-azure-sentinel"></a>Azures säkerhets bas linje för Azure Sentinel

Den här säkerhets bas linjen använder vägledning från [Azures säkerhets benchmark-version 1,0](../security/benchmarks/overview-v1.md) till Azure Sentinel. Azure Security Benchmark ger rekommendationer om hur du kan skydda dina molnlösningar i Azure.
Innehållet grupperas efter **säkerhets kontrollerna** som definieras av Azures säkerhets benchmark och relaterade rikt linjer som gäller för Azure Sentinel. **Kontroller** som inte gäller för Azure Sentinel har uteslutits.

 
Om du vill se hur Azure Sentinel fullständigt mappar till Azures säkerhets benchmark kan du läsa mer i den [fullständiga mappnings filen för Azure Sentinel Security](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
**Vägledning**: underhålla en inventering av de användar konton som har administrativ åtkomst till kontroll planet (till exempel Azure Portal) på Azure Sentinel-arbetsytan. 

Du kan använda fönstret identitets-och åtkomst kontroll i Azure Portal för din prenumeration för att konfigurera rollbaserad åtkomst kontroll i Azure (Azure RBAC). Rollerna tillämpas på användare, grupper, tjänstens huvud namn och hanterade identiteter i Azure Active Directory (Azure AD). Azure Sentinel använder också Azure RBAC för att tillhandahålla inbyggda administrativa roller, till exempel Azure Sentinel Contributor, som kan tilldelas användare, grupper och tjänster i Azure. 
...
**Vägledning**: kontroll Plans åtkomst för Azure Sentinel-arbetsytan (t. ex. Azure Portal) är tillgänglig via REST API och stöder SSO. Du autentiserar genom att ange Authorization-huvudet för dina begär anden till en JSON Web Token som du får från Azure Active Directory (Azure AD).
...
**Vägledning**: Använd en privilegie rad åtkomst arbets Station (Paw) med Azure AD multifaktor-autentisering aktive rad för att logga in och konfigurera dina Azure Sentinel-relaterade resurser. 
...
Du har åtkomst till Azure AD-inloggning, gransknings-och risk händelse logg källor, som gör att du kan integrera med Azure Sentinel eller en system informations-och händelse hanterings lösning från tredje part.
...
**Vägledning**: implementera separata prenumerationer om du vill kan du även ta med hanterings grupper för arbets ytor för utveckling, testning och produktions kontroll.
...
- [Översikt över Azure Monitor kundhanterad nyckel](https://docs.microsoft.com/azure/azure-monitor/logs/customer-managed-keys#customer-managed-key-overview) 
...
- [Så här strömmar du aviseringar till Azure Sentinel](connect-azure-security-center.md)
