...
![Diagram för Azure Resource Manager](media/azure-stack-edge-gpu-connect-resource-manager/edge-device-flow.svg)
...
Innan du börjar ska du kontrol lera att klienten som används för att ansluta till enheten via Azure Resource Manager använder TLS 1,2. Mer information finns i [Konfigurera TLS 1,2 på Windows-klienten som har åtkomst till Azure Stack Edge Pro-enhet](azure-stack-edge-gpu-configure-tls-settings.md).
...
3. Använd följande bild som referens. Spara **värdens** fil.
    ...
    ![Hosts-fil i anteckningar](media/azure-stack-edge-gpu-connect-resource-manager/hosts-file-notepad.png)
...
1. Du kan använda kommando rads verktyget ping.exe för att kontrol lera att slut punkts namnet är löst. Med en IP-adress returnerar Ping-kommandot TCP/IP-värdnamnet för datorn som du \' spårar.
    ...
    ![Ping i kommando tolken](media/azure-stack-edge-gpu-connect-resource-manager/ping-command-prompt.png)
...
2. Anropa lokala enhets-API:er för att autentisera anslutningarna till Azure Resource Manager. 
    ...
    2. Du kan ansluta via `login-AzureRMAccount` eller via `Connect-AzureRMAccount` kommando. 
        ...
        1. Logga in genom att skriva följande kommando. Klient-ID: t i den här instansen är hårdkodat-c0257de7-538f-415c-993a-1b87a031879d. Använd följande användar namn och lösen ord.
            ...
            - **Lösen ord**  -  [Ange lösen ordet för Azure Resource Manager](azure-stack-edge-gpu-set-azure-resource-manager-password.md) och Använd det här lösen ordet för att logga in. 
