---
author: aahill
ms.author: aahi
ms.service: cognitive-services
ms.topic: include
ms.date: 03/20/2019
ms.openlocfilehash: 41ac1478b1028a847fc0d5e7e70802375e910837
ms.sourcegitcommit: f28ebb95ae9aaaff3f87d8388a09b41e0b3445b5
ms.translationtype: MT
ms.contentlocale: sv-SE
ms.lasthandoff: 03/30/2021
ms.locfileid: "95999117"
---
> [!NOTE]
> För [bästa resultat](../articles/cognitive-services/anomaly-detector/concepts/anomaly-detection-best-practices.md) när du använder API: t för avvikelse detektor bör JSON-formaterade tids serie data innehålla:
> * data punkter avgränsade med samma intervall, med högst 10% av det förväntade antalet punkter som saknas.
> * minst 12 data punkter om dina data inte har ett tydligt säsongs mönster.
> * minst fyra mönster förekomster om dina data har ett tydligt säsongs mönster. 
